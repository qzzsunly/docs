...
Spring 提供一个校验器接口 <<validator,Validator interface>>, 可以用来在应用程序的所有层中进行校验. 在 Spring MVC 中你可以配置一个全局的 `Validator` 实例, 只要在controller 方法的参数中出现了 `@Valid` 或者 `@Validated` 注解，它就能生效, 或者通过controller 中的`@InitBinder` 方法配置局部
`Validator` . 全局和局部的 validator 实例组合起来就可以实现复杂的校验。

Spring 通过 `LocalValidatorFactoryBean`也支持 <<validation-beanvalidation-overview,supports JSR-303/JSR-349>> Bean Validation。
 `LocalValidatorFactoryBean` 使 Spring `org.springframework.validation.Validator`
接口适应 Bean Validation `javax.validation.Validator` 的协议 。接下来描述了这个类可以作为一个全局的validator 插入到 Spring MVC 中。

在`@EnableWebMvc` 或者 `<mvc:annotation-driven>` 默认用法中，当发现 classpath 中有Bean Validation 的实现时，比如 Hibernate Validator ,  将自动通过`LocalValidatorFactoryBean`在Spring MVC 中注册 Bean Validation 支持。

[NOTE]
====
有时候在controller 或者其他类中注入 `LocalValidatorFactoryBean`是很方便的.最简单的方式是申明自己的 `@Bean` 并且标记 `@Primary` 来防止与MVC Java 提供的 bean 产生冲突。

如果你喜欢使用MVC Java 配置提供的 bean , 你需要重写 `WebMvcConfigurationSupport` `mvcValidator` 方法，明确申明该方法返回 `LocalValidatorFactory` 而不是 `Validator`。更多信息请查看 <<mvc-config-advanced-java>>
关于如何切换到扩展提供的配置。
====

配置自己的全局 `Validator` 有二种方式:
...
在这样的最简配置下，任何时候在方法的参数中发现 `@Valid` 或者 `@Validated` , 配置的校验器将会对这个参数进行校验. 任何校验不通过的行为都将作为错误自动显露到 `BindingResult` 中，`BindingResult` 可以作为一个方法的参数来进行访问，也可以渲染到 Spring MVC HTML 视图中.



[[mvc-config-interceptors]]
=== 拦截器
你可以配置 `HandlerInterceptors` or `WebRequestInterceptors` 并运用到所有进入的请求或者限定到特定的URL路径模式.
...
=== 内容协商
你可以配置告诉Spring MVC 如何确定请求中的媒体类型。
可用的方式是对文件后缀检查URL路径，检查"Accept" 头，特定的请求参数，或者当没请求到任何内容的时候返回一个默认的内容类型。默认的方式是先检查请求的URI路径后缀 ，再检查"Accept" 头。

MVC Java 配置 和  MVC 命名空间默认会在classpath发现相关的依赖时注册 `json`, `xml`, `rss`, `atom`. Additional
额外的路径后缀到媒体类型的映射可能需要明确地注册，并且为了达到RFD 攻击检测(更多细节请查看 <<mvc-ann-requestmapping-rfd>> )的目的，需要将它加入到白名单中作为安全的后缀。 .

下面是一个通过MVC Java 配置来自定义内容协商的例子:
...
在 MVC 命名空间中, `<mvc:annotation-driven>` 有一个
`content-negotiation-manager` 属性, 希望一个 `ContentNegotiationManager`。
它可以被 `ContentNegotiationManagerFactoryBean` 创建:
...
如果没有使用 MVC Java 配置 或者 MVC 命名空间, 你需要创建一个
 `ContentNegotiationManager` 的实例，并且你需要使用它配置 `RequestMappingHandlerMapping`来做请求处理,  `RequestMappingHandlerAdapter` 和
`ExceptionHandlerExceptionResolver` 来做内容协商.
